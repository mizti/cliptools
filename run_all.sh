...
#   --engine       : STT エンジン (whisperkit|azure) ※省略時は WhisperKit
...
LOCALE="en-US"
ENGINE=""          # whisperkit|azure。空なら generate_srt.sh 側のデフォルトに任せる
CLIP_STARTS=()
CLIP_ENDS=()
AUDIO_ONLY=false
REENCODE=false
...
if [[ -n "$ENGINE" && ! "$ENGINE" =~ ^(whisperkit|azure)$ ]]; then
  echo "Error: --engine は whisperkit|azure で指定してください" >&2
  exit 1
fi
