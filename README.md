...
2. **WhisperX（デフォルト）/ Azure Speech (STT) による自動文字起こし & 文単位 SRT 生成**（`generate_srt.sh`）
3. **英語 SRT の固有名詞補正（デフォルト: Ollama + qwen3、オプションでAzure OpenAI）**（`fix_unique_nouns.py`）
4. **英語 SRT → 日本語 SRT 翻訳（デフォルト: Ollama + qwen3、オプションでAzure OpenAI）**（`translate_srt.sh` / `translate_srt_gpt.py`）
...
### 2-1-2. WhisperX（デフォルト: ローカルSTT）を使うためのセットアップ
...
`generate_srt.sh` を **Azure Speech STT** で動かす場合のみ必要です（デフォルトは WhisperX）。
...
このリポジトリでは、必要に応じて各種設定を `.env` にまとめて管理します。

デフォルト（Ollama）で固有名詞補正／翻訳をするだけなら Azure 系の設定は不要です。

```bash
# ====== Local LLM (Ollama) ======

# 省略時もデフォルトは ollama
export CLIPTOOLS_LLM_BACKEND=ollama
...
# Azure Speech STT を使って字幕生成したい場合（デフォルトは WhisperX）
./run_all.sh -f clips/workdir/input.mp4 -o clips/workdir --engine azure -n 1
```
